...
@("StaticSparseSet")
@safe unittest {
    // describe("Compile time detection of the Type"),
    {
        // given("A type T that is a valid StaticSparseSet")
        alias Set = StaticSparseSet!(uint, 16);

        // expect("that is detected as a SparseSet")
        isSparseSet!Set.should.be.True;

        // and("that is detected as StaticSparseSet")
        isStaticSparseSet!Set.should.be.True;

        // and("but not as a DynamicSparseSet")
        isDynamicSparseSet!Set.should.be.False;

        // and("the expect capacity and max value is the incated by the type")
        auto set = Set();
    ...
    // describe("Inserting and searching a value over the max value"),
    {
        ...
        set.search( 33).should.be.equal( -1); // Not found becasue isn't inserted
        set.empty.should.be.True;
    }
...
unittest {
    // describe("Compile time detection of the Type"),
    {
        // given("A type T that is a valid DynamicSparseSet")
        alias Set = DynamicSparseSet!uint;

        // expect("that is detected as a SparseSet")
        isSparseSet!Set.should.be.True;

        // and("that is detected as DynamicSparseSet")
        isDynamicSparseSet!Set.should.be.True;

        // and("but not as a StaticSparseSet")
        isStaticSparseSet!Set.should.be.False;
    }
    ...
    // describe("Using the default constructor, gives a empty set"),
    {
        ...
        // and("the capacity and maxValue are the same indicated on the constructor")
        set.capacity.should.be.equal(32);
        set.maxValue.should.be.equal(64);
    }
    ...
    // describe("The SparseSet can grow")
    {
        // given("A empty set")
        auto set = Set(8, 64);

        // when("We reserve a new size")
        set.reserve(10);

        // then("the set expands")
        set.capacity.should.be.equal(10);

        // when("We insert more values that the actual capacity")
        import std.range : iota;
        set.insert(iota(0, 20));

        // then("the SparseSet grows to have enought capacity")
        set.length.should.be.equal(20);
        set.capacity.should.be.equal(25);
    }

    // describe("The SparseSet maxValue can grow")
    {
        // given("A empty set")
        auto set = Set(8, 8);

        // when("We set a new maxValue a new size")
        set.maxValue = 10;

        // then("maxValue comes the exact value")
        set.maxValue.should.be.equal(10);
    }
...
unittest {
    ...
    alias DSet = DynamicSparseSet!uint;
    ...
    // describe("Passing by reference a Set")
    {
        ...
        auto dset = DSet(16, 16, [ 1, 4, 2, 3, 5, 4, 10]);
